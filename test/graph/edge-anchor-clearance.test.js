 ...
 *
 * `GraphEdgeAnchors` puts its source ring on `plan.source`, which for the
 * commonest edge on the canvas — a plain card leaving by its bottom border — is
 * exactly where the card's drag grip sits. The ring carries an invisible grab
 * disc wider than that grip, so left alone it takes every press meant for the
 * grip: with an outgoing edge selected, no new connection can be drawn out of
 * the card. `clearOf` steps the anchor out along the edge instead, and
...
import { clearOf } from '../../src/graph/GraphEdgeAnchors';
import { handleAt } from '../../src/graph/source-handles';

// A card at (0, 0), 200×80, with the anonymous drag grip centred on its
// bottom border — the measurements React Flow reports for a plain card.
...
describe( 'clearOf', () => {
	...
	it( 'clears an exit badge by its own size, not a fixed nudge', () => {
		// A named exit's badge is a different size from the anonymous grip, so
		// a constant step would leave the ring still on top of one.
