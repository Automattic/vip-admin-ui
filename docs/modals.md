...
A modal is a dialog that holds the user's attention until they finish or
dismiss it. Every modal is the WPDS `Modal` from `@wordpress/components`, and
these rules set the title, width, body and footer around it, so every modal is
sized, spaced and ordered the same way. See [Actions](actions.md) for button
weight and labels, and [Settings](settings.md) for screen layout.
...
| Concern | Rule |
|---|---|
| Header title | A plain translated string in `title`. Not JSX. |
| Header actions | In the `headerActions` prop, not inside `title`. |
...
| Width | Only the `size` prop: `small` / `medium` / `large` / `fill`. No CSS `max-width`/`min-width`, and no inline `minWidth`. |
| Footer | `<ModalActions>`. Not a raw `<div>` or a custom `Stack`. |
| Button order | Cancel on the left, primary on the right. The primary is rightmost. |
| Button variants | Cancel is `tertiary`, the action is `primary`, and a destructive action is `primary` + `isDestructive`. |
| Pending | `isBusy` + `disabled`, and the label does not change. Not `{ busy ? <Spinner/> : label }`. |
...
| Form controls | `__next40pxDefaultSize` + `__nextHasNoMarginBottom` on every control. |
| Dismissing | Dismissible by default. Only a blocking modal changes the three dismiss props. |

## Header

The `Modal` builds its header from props, so do not build one in the body.
JSX in `title` breaks the fixed-height header. A button next to the close
...
short and do not. This package does not ship a class for this yet, so use your own
modifier class:
...
Do not use `.components-modal__content { max-width: … }`, `style={ { maxWidth: … } }`
...
beside it. The footer handles its own layout. Do not add `className` to the
buttons for spacing, do not use `margin-right: auto`, and do not write footer
CSS for a single modal. If you need something different, add a prop to
`ModalActions`.

## Body: `<ModalBody>`

A vertical `Stack` that spaces the content blocks, so children need no margins
and no extra `Stack`s. Use `Text` for prose, not raw `<p>`, `<h4>` or `<span>`.
...
same. Use a `Spinner` only while the modal's body content is loading, not as a
button's label.
...
**Dismissing.** Keep the `Modal` defaults. A blocking modal, one the user has to
resolve before moving on, sets all of the following. It is the only reason to
change these props:
...
Use this instead of `window.confirm()`, which cannot be styled or made
accessible to match the admin. It returns an async `confirm( message, options )` that
...
- Labels you do not set, and a missing `title`, fall back to the `confirm` and
  `cancel` strings from `<StringsProvider>` (see [Strings](i18n.md)). Give a
  destructive confirmation a real title.
- It renders a `small` `Modal` with a `tertiary` Cancel and a `primary` confirm
  ...
  waiting, that one resolves `false`, so no caller is left waiting.

`useConfirm` shows two buttons, so it is only for yes/no questions. An
