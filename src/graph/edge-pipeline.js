 ...
 *    which take the borders the first edge of their pair chose (planned
 *    independently they can disagree, and the pair crosses), and except an
 *    edge leaving a start terminal, whose departure is not routed at all: it is
 *    pinned to the terminal's one visible handle (`pinToExit`);
 * 2. ports sharing a border are ordered and spaced (`spreadPorts`), and short
 *    near-column runs are straightened;
 * 3. co-travelling edges are gathered into lanes (`bundleEdges`), the nodes'
 ...
 *
 * A node's named exits follow the ports, not the other way around: the exits
 * have a declared order, and when the pipeline puts the second exit's port left
 * of the first's, the two edges would cross under the node purely to reach the
 * badge they belong to. The order on the card is a convention; the association
 * that matters is between an edge and its own badge, so the pipeline reports
 * which order the named-exit edges actually leave in (`portOrder`) and the
 * badges reorder to match (`exitOrder`).
 *
 * @package
 */
...
/**
 * Pin an edge's departure to a start terminal's own handle.
 *
 * A start terminal is not a card. It draws exactly one exit — a small circular
 * handle centred on its bottom border (`GraphTerminal`) — and that dot is where
 * the flow leaves. Left to the router it would be costed on all four borders
 * like any other node, and the port would land wherever the first node
 * happened to sit, so the one departure point on the canvas that is drawn
 * wandered away from the mark drawing it.
 *
 ...
 * within a face shared by two or more ports, and the terminal's bottom face
 * carries only this one.
 *
 * The socket mark is dropped for the same reason, by the same rule every other
 * mark landing on a handle follows (`GraphEdgeOverlay`) — the handle already
 * says where the edge attaches, and a second mark domed over it says it twice.
 *
 * An end terminal is deliberately not the mirror of this: it has no source
 * handle of its own, and the arrowhead arriving at it is the only thing that
 * says the flow finishes there.
 *
 * @param {Object} planned The planned edge (`plan`, `obstacles`, `own`).
 * @param {Object} rect    The start terminal's rectangle.
 */
function pinToExit( planned, rect ) {
 ...
 *                        `sourceHandle`).
 * @param {Object} rects  Measured node rectangles by id. A rect with
 *                        `pinnedExit` set is a start terminal (`pinToExit`).
 ...
 *         Drawn plans by edge id, named-exit port order by node id, and whether
...
export function buildEdgePlans( edges, rects, memory, now ) {
	...
	// Fan coherence: every edge between the same pair of nodes takes
	...
	edges.forEach( ( edge ) => {
		...
		const planned = {
			...
			exit: edge.sourceHandle || null,
			plan,
			obstacles,
			own: [ source, target ],
		};
		if ( source.pinnedExit ) {
			pinToExit( planned, source );
		}
		plans.push( planned );
	} );
	...
	// Which exit now leaves from which side, so the badges can follow the
	...
	// Drawn last, and stopped short of the node it arrives at so the arrowhead
	// has the clearance the design gives it — see `trimPathEnd`. Everything above
...
/**
 * The order a node's named exits sit in, taken from where their edges actually
 * leave. An exit without an edge keeps its declared place, so a node with one
 * edge out still reads in the order its exits were declared.
 *
 * @param {Array}  exits  Exit ids, in declared order.
 * @param {?Array} routed Exit ids in port order, from `buildEdgePlans`.
 * @return {Array} All exit ids, in display order.
 */
export function exitOrder( exits, routed ) {
	if ( ! routed || routed.length < 2 ) {
		return exits;
	}
	const placed = routed.filter( ( id ) => exits.includes( id ) );
	return placed.concat( exits.filter( ( id ) => ! placed.includes( id ) ) );
}
