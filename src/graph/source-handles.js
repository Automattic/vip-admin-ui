 ...
 * with what is underneath them. `GraphEdgeOverlay` asks so it can leave a
 * socket undrawn where the handle already reads as the port;
 * `GraphEdgeAnchors` asks so a grab ring does not plant an invisible disc over
 * the control it covers. Both questions are the same measurement, and it is
 * React Flow's own — read from the store rather than restated from the
 * stylesheet, so the drag grip's pill and a node's cluster of named exits are
 * whatever they actually render as, including after the cluster reorders to
 * follow its edges' ports (`edge-pipeline.js`).
 *
 * @package
 */
 ...
 * for the anonymous drag grip — because the arrowhead rule needs to tell a
 * named exit from the grip, and a named exit's handle id is the exit's id.
 *
 ...
 * way needs to know how far — see `GraphEdgeAnchors`, which pushes its source
 * ring past the covering handle's far edge.
 *
 ...
 * over some *other* node's grip is the overlapping-cards case the overlay
