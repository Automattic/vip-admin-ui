 ...
 * two nodes sitting side by side would leave the bottom of one and enter the
 ...
 * (`inflections` — an S between two nodes is nearly always a port that should
 ...
 *
 * There is deliberately no obstacle router. A third node in the way is
 ...
 * swinging wide around three stacked nodes is not.
 *
 * @package
 */
 ...
 * that passes a node widely is left alone; one that comes near its centre is
 ...
 * nodes add, so an edge threading a row of nodes settles where they balance.
 *
...
export function repulsion( a, b, rects ) {
	...
	rects.forEach( ( r ) => {
		...
		// vertically than horizontally — a node a whole rank away in y was
...
export function routeCost( plan, source, target ) {
	...
	// Only between ranks: two nodes side by side on one rank have nothing but
...
export function attempt( params, obstacles, own ) {
	...
	return {
		...
		// Its own two nodes, counted separately: an edge may legitimately
		// pass near a third node, but it never has to pass under the one it
 ...
 * that is how every edge in a fan takes the borders the first of them
...
export function planEdge( { source, target, obstacles = [], sides = null } ) {
	...
	// node is dragged past the point where two pairings cost the same.
