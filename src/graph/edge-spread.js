 ...
 * edge leaving a node for somewhere below it lands on the same point of the
 * same border. Five edges out of one node then leave from one spot and
 * braid together for the first hundred pixels, and which line goes where is
 * unreadable exactly where it matters most — at the node the reader is
 * looking at.
 *
 ...
 * by where their edges are actually *going* — the far node's centre, which
 ...
 * ports open out, loose ones pull in, and a fan out of a node reads as one
 * set of exits rather than a few unrelated ones.
 *
 * Finally, short edges are brought into line: two nodes nearly but not quite
...
export function spreadPorts( plans, rects ) {
	...
	Object.values( groups ).forEach( ( g ) => {
		...
		// The far *node*, not the far port or the control point. Both of
		...
		// node's centre owes nothing to any of this.
		const farOf = ( it ) =>
