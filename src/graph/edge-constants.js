 ...
 *
 * Every value here came out of a routing lab where each was a live slider
 * judged against seven scenarios: a real production graph, back edges, a
 * multi-exit fan-out, a parallel fan, a reciprocal pair, a crowded rank, and a
 * jump across bands. They are one
 ...
 * routing *around* nodes reads worse than passing behind them — a line under
 * a card is legible when the stroke breaks to say so (`edge-tunnel.js`), while
 * a line swinging wide around three stacked nodes is not. So there is no
 ...
 * node is entered from above, and an arrowhead into a flank reads as a
...
/**
 * An edge never has to pass under the node it starts from or ends at — moving
 ...
 * near-ties, so an edge doesn't flicker between borders while its node is
 ...
 * so dragging a node near an edge in x but a full rank away in y still
 ...
 * control point 226px beyond the node it was going to.
 *
 ...
 * its shape and drops out of the lane order, which is the honest result: it was
 ...
 *
 * One number for both marks the port design puts near a border: the
 * arrowhead's tip, and the dome of the cup closing an underpass.
 ...
 *
 * The design draws its dots as zero-length dashes under a round cap; these are 1px
 * dashes under a butt cap, keeping the design's 3.1px rhythm and its ink. The
 ...
 * zoomed in and, at the zoom a whole graph fits on screen at, thinner than
 * the pointer can reliably be aimed, so the same edge is easy to select at one
 * zoom and fiddly at another. `GraphEdge` divides it by the live zoom to
