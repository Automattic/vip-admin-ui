/**
 * Underpass breaks — where an edge passes behind a node, the stroke says so.
 *
 ...
 * the hidden span is drawn faintly and dotted on a layer above the cards.
 *
...
/**
 * Where a curve runs beneath a node, measured along the curve.
 *
...
function tunnelDraw( geo, capLen ) {
	...
	// which side the node is on and reads as the mouth the line goes into.
	//
...
export function tunnelAll( plans ) {
	...
	plans.forEach( ( p ) => {
		...
		// Its own two nodes count as well. An edge that bows back over the
