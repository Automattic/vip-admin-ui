/**
 * GraphContextMenu — the canvas's right-click menu.
 *
 ...
 * blur. `GraphCanvas` owns *whether* it's open and at what coordinate; this
 * owns the rest.
 *
 ...
 * from on the way out, since the thing that opened it (a band label, or the
 * pane) is where the author was.
 *
 * One level only. An item that would need to ask "which node?" belongs in an
 * inspector, not in a submenu.
 *
 * @package
 */
...
/**
 * @param {Object}   props
 * @param {number}   props.x       Left, in px from the canvas's own box.
 * @param {number}   props.y       Top, in px from the canvas's own box.
 * @param {Array}    props.items   `[ { id, label, icon?, disabled?, onSelect } ]`.
 * @param {string}   props.label   The menu's accessible name: what it acts on.
 * @param {Function} props.onClose Called on dismissal, and before an item runs.
 * @return {JSX.Element} The menu.
 */
export function GraphContextMenu( { x, y, items, label, onClose } ) {
	...
	const restoreFocus = useCallback( () => {
		...
		if ( typeof opener?.focus === 'function' && opener.isConnected ) {
			opener.focus();
		}
	}, [] );
		...
		>
			{ items.map( ( item, index ) => (
				<button
					...
					type="button"
					role="menuitem"
					className="vipui-graph-menu__item"
				...
				>
					{ item.icon && (
						<Icon
							icon={ item.icon }
							size={ 16 }
							className="vipui-graph-menu__icon"
						/>
					) }
