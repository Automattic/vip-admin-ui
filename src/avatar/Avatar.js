...
/**
 * What each size can hold, keyed by the WPDS dimension scale.
 *
...
export function Avatar( { src, name, icon, size = 'sm', className = '' } ) {
	...
	const classNames = [ 'vipui-avatar', `vipui-avatar--${ size }`, className ]
		.filter( Boolean )
		.join( ' ' );
	...
	return (
		<BaseAvatar.Root className={ classNames }>
			<BaseAvatar.Image
				src={ src }
				alt=""
				className="vipui-avatar__image"
			/>
			<BaseAvatar.Fallback className="vipui-avatar__initials">
				{ icon ? (
