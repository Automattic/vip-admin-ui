 ...
 *
 * `InspectorFieldList` and `InspectorChoiceRow` build their rows from these same
 * primitives rather than growing near-identical lists of their own — "a line
 * that names something and opens its options" is one shape, and two copies of
 * it drift.
 *
 ...
 * unlike the rows that open what they report. This gives one a real trigger: a
 * button, which opens on hover, on focus and on tap, rather than a hover target
 * on label text that a keyboard or a touch screen can never reach.
 *
...
export function InfoTip( { about, children } ) {
	...
	return (
		<Tooltip.Provider>
			<Tooltip.Root>
				<Tooltip.Trigger
					className="vipui-inspector-infotip"
					aria-label={ strings.aboutSetting( about ) }
				>
					<Icon icon={ info } size={ 16 } />
				</Tooltip.Trigger>
				{ /* Left, because an inspector docks to the right edge of the
				     screen and a tooltip to its right would open off-screen. */ }
				<Tooltip.Popup
					className="vipui-inspector-infotip__popup"
 ...
 *
 * `children` slots in front of the name — a colored dot, an icon.
 *
 * The value can be an absence — something nobody has set yet — and `empty`
 * marks it, so the placeholder standing in for it doesn't read as the setting
 * itself.
 *
 * @param {Object}      props            Component props.
...
function FactBody( { onSelect, selectLabel, render, expanded, ...content } ) {
	...
	return (
		<Stack
			...
			align="center"
			gap="sm"
			className="vipui-inspector__fact-open"
 ...
 *
 * The shape every summary in an inspector uses, so facts, exits and lists of
 * fields read as one grammar rather than three inventions. Render inside a
 ...
 *
 * @param {Object}      props             Component props.
 * @param {string}      [props.className] Extra classes for the row, e.g. `is-disabled`.
...
export function Fact( { className, trailing, ...content } ) {
	return (
		<Stack
			...
			align="center"
			gap="sm"
			className={ [ 'vipui-inspector__fact', className ]
				.filter( Boolean )
				.join( ' ' ) }
		>
			<FactBody { ...content } />
 ...
 *
 * Must render inside a dnd-kit `SortableContext`.
 *
 * @param {Object}      props             Component props.
...
export function SortableFact( {
	...
	return (
		// wpds-allow system/inline-style -- dnd-kit's per-frame drag transform; no prop or stylesheet can carry a runtime-measured value
		<Stack
			render={ <li /> }
			align="center"
			gap="sm"
		...
		>
			<button
				type="button"
				className="vipui-inspector__grip"
